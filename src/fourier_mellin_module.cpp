...
template <> cv::Mat numpy_to_mat<0>(const py::array_t<float> &input) {
  py::buffer_info buf = input.request();
  int type;
...
py::array_t<float> mat_to_numpy(const cv::Mat &mat) {
  ...
  if (mat.isContinuous()) {
    ...
    for (int i = 0; i < rows; ++i) {
      std::memcpy(data.data() + i * cols * channels, mat.ptr<float>(i),
                  cols * channels * sizeof(float));
    }
  }

  return py::array_t<float>({rows, cols, channels},
                            {cols * channels * sizeof(float),
                             channels * sizeof(float), sizeof(float)},
                            data.data());
}
...
PYBIND11_MODULE(MODULE_NAME, m) {
  py::class_<Transform>(m, "Transform")
      .def(py::init<>())
      .def(py::init([](double x, double y, double scale, double rotation,
                       double response) {
        return new Transform{x, y, scale, rotation, response};
      }))
      .def("__repr__",
           [](const Transform &t) {
             // TODO: use the operator<< format previously defined instead for
             // consistency
             return "<" TOSTRING(MODULE_NAME) ".Transform x_offset=" +
                    to_string_with_precision(t.x, 2) +
                    ", y_offset=" + to_string_with_precision(t.y, 2) +
                    ", rotation=" + to_string_with_precision(t.rotation, 2) +
                    ", scale=" + to_string_with_precision(t.scale, 2) +
                    ", response=" + to_string_with_precision(t.response, 2) +
                    ">";
           })
      ...
      .def(
          "__mul__",
          [](const Transform &a, const Transform &b) { return a * b; },
          py::is_operator())
      .def(
          "get_matrix",
          [](const Transform &a) {
            cv::Mat matrix = a.GetMatrix();
            return py::array_t<double>({matrix.rows, matrix.cols},
      ...
      .def(
          ...
          "Get Inverse Matrix")
      .def(
          "get_inverse", [](const Transform &a) { return a.GetInverse(); },
          "Get Inverse Transform")
      .def("to_dict", [](const Transform &t) {
        return py::dict("x"_a = t.x, "y"_a = t.y, "scale"_a = t.scale,
                        "rotation"_a = t.rotation, "response"_a = t.response);
      });
  ...
  py::class_<FourierMellin>(m, "FourierMellin")
      ...
      .def(
          "register_image",
          [](const FourierMellin &fm, std::string_view target_fp) -> auto {
            auto transform = fm.RegisterImage(target_fp);
            return transform;
          },
          "Register target image to reference.")
      .def(
          "register_image",
          [](const FourierMellin &fm, py::array_t<float> target) -> auto {
            auto targetMat = numpy_to_mat<0>(target).clone();
            auto transform = fm.RegisterImage(targetMat);
            return transform;
          },
          "Register target image to reference.");
  // .def("register_image", [](const FourierMellin& fm, std::string_view
  // target_fp) -> auto {
  //             auto[transformed, transform] =
  //             fm.GetRegisteredImage(target_fp); return
  //             std::make_tuple(mat_to_numpy(transformed), transform); },
  //             "Register target image to reference and return aligned target")
  // .def("register_image", [](const FourierMellin& fm, py::array_t<float>
  // target) -> auto {
  //     auto targetMat = numpy_to_mat<0>(target).clone();
  ...
  m.def(
      "get_transformed",
      [](const py::array_t<float> &img, Transform transform) {
