...
LogPolarMap::LogPolarMap(int width, int height)
    : width_(width), height_(height), logPolarSize_(0), logBase_(0.0) {
  ConstructMaps();
}

cv::Mat LogPolarMap::ConvertToLogPolar(const cv::Mat &img) const {
  cv::Mat log_polar;
  cv::remap(img, log_polar, xMap_, yMap_, cv::INTER_LANCZOS4,
...
int LogPolarMap::GetLogPolarSize() const { return logPolarSize_; }

double LogPolarMap::GetLogBase() const { return logBase_; }
