...
FourierMellin::FourierMellin(const cv::Mat &reference)
    : width_(reference.size().width), height_(reference.size().height),
      logPolarMap_(width_, height_), imageFilter_(width_, height_) {
  ...
  if (referenceImg_.channels() == 3) {
    ...
    throw std::runtime_error(
        std::string("Could not convert to LogPolar with ") +
        std::to_string(referenceImg_.channels()) +
        " channels. (Only single-channel images.)");
  }
  referenceImgLogPolar_ = ConvertImageToLogPolar(referenceImg_);
}

FourierMellin::FourierMellin(std::string_view reference_fp)
    : FourierMellin(ReadGrayscaleImageFromFile(reference_fp)) {}

Transform FourierMellin::RegisterImage(const cv::Mat &target) const {
  if (target.channels() != 1) {
    throw std::runtime_error(std::string("Could not register image with ") +
  ...
  auto [logScale, logRotation] =
      cv::phaseCorrelate(logPolarTarget, referenceImgLogPolar_, cv::noArray(),
                         &responseScaleRotation);
  ...
  cv::warpAffine(referenceImg_, rotatedReference, rotationMatrix,
                 referenceImg_.size());

  double response;
  auto [xOffset, yOffset] =
      cv::phaseCorrelate(target, rotatedReference, cv::noArray(), &response);
...
FourierMellin::GetRegisteredImage(const cv::Mat &target) const {
...
std::tuple<cv::Mat, Transform>
FourierMellin::GetRegisteredImage(std::string_view target_fp) const {
  auto target = ReadGrayscaleImageFromFile(target_fp);
...
cv::Mat
FourierMellin::ReadGrayscaleImageFromFile(std::string_view img_fp) const {
